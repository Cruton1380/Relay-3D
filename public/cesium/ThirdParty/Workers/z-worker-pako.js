!function() {
  ...
  class be extends g {
    constructor(e2, { chunkSize: t2, CompressionStream: n2, CompressionStreamNative: s2 }) {
      ...
      a2 && !i2 || !c2 || (p2 = new C(), d2 = ze(d2, p2)), r2 && (d2 = ke(d2, o2, { level: l2, chunkSize: t2 }, s2, n2)), a2 && (i2 ? d2 = ze(d2, new he(e2)) : (f2 = new ne(e2), d2 = ze(d2, f2))), ve(h2, d2, () => {
        let e3;
  ...
  class Se extends g {
    constructor(e2, { chunkSize: t2, DecompressionStream: n2, DecompressionStreamNative: s2 }) {
      ...
      let p2, f2, d2 = super.readable;
      o2 && (a2 ? d2 = ze(d2, new ue(e2)) : (f2 = new te(e2), d2 = ze(d2, f2))), l2 && (d2 = ke(d2, h2, { chunkSize: t2 }, s2, n2)), o2 && !a2 || !i2 || (p2 = new C(), d2 = ze(d2, p2)), ve(this, d2, () => {
        if ((!o2 || a2) && i2) {
  ...
  function ve(e2, n2, s2) {
    n2 = ze(n2, new g({ flush: s2 })), t.defineProperty(e2, "readable", { get: () => n2 });
  }
  function ke(e2, t2, n2, s2, r2) {
    try {
      e2 = ze(e2, new (t2 && s2 ? s2 : r2)(_e, n2));
    } catch (s3) {
      if (!t2) throw s3;
      e2 = ze(e2, new r2(_e, n2));
    }
    return e2;
  }
  function ze(e2, t2) {
  ...
  const De = "data", Ce = "close";
  class Ie extends g {
    constructor(e2, n2) {
      super({});
      const s2 = this, { codecType: a2 } = e2;
      let o2;
      a2.startsWith("deflate") ? o2 = be : a2.startsWith("inflate") && (o2 = Se), s2.outputSize = 0;
      let i2 = 0;
  ...
  const qe = new l(), Re = new l();
  let He, Pe = 0, Be = true;
  async function Ke(e2) {
    try {
      const { options: t2, scripts: s2, config: r2 } = e2;
      if (s2 && s2.length) try {
        Be ? importScripts.apply(b, s2) : await Te(s2);
      } catch (e3) {
        Be = false, await Te(s2);
      }
      ...
      const a2 = { highWaterMark: 1 }, o2 = e2.readable || new w({ async pull(e3) {
        const t3 = new h((e4) => qe.set(Pe, e4));
        Ve({ type: "pull", messageId: Pe }), Pe = (Pe + 1) % n.MAX_SAFE_INTEGER;
        const { value: s3, done: r3 } = await t3;
        e3.enqueue(s3), r3 && e3.close();
      } }, a2), i2 = e2.writable || new y({ async write(e3) {
        let t3;
        const s3 = new h((e4) => t3 = e4);
        Re.set(Pe, t3), Ve({ type: De, value: e3, messageId: Pe }), Pe = (Pe + 1) % n.MAX_SAFE_INTEGER, await s3;
      } }, a2), c2 = new Ie(t2, r2);
      He = new AbortController();
      const { signal: l2 } = He;
      await o2.pipeThrough(c2).pipeThrough(new Ae(r2.chunkSize)).pipeTo(i2, { signal: l2, preventClose: true, preventAbort: true }), await i2.getWriter().close();
      const { signature: u2, inputSize: p2, outputSize: f2 } = c2;
      Ve({ type: Ce, result: { signature: u2, inputSize: p2, outputSize: f2 } });
    } catch (e3) {
      e3.outputSize = 0, xe(e3);
    }
  }
  ...
  function xe(e2 = new r("Unknown error")) {
  ...
  addEventListener("message", ({ data: e2 }) => {
    ...
    try {
      if ("start" == t2 && Ke(e2), t2 == De) {
        const e3 = qe.get(n2);
        qe.delete(n2), e3({ value: new a(s2), done: r2 });
      }
      if ("ack" == t2) {
        const e3 = Re.get(n2);
        Re.delete(n2), e3();
    ...
    const { Deflate: e2, Inflate: t2 } = ((e3, t3 = {}, n2) => ({ Deflate: Ee(e3.Deflate, t3.deflate, n2), Inflate: Ee(e3.Inflate, t3.inflate, n2) }))(pako, { deflate: { raw: true }, inflate: { raw: true } }, (e3, t3) => e3.onData = t3);
