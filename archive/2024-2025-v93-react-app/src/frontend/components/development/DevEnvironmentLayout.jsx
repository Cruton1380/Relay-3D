...
import './DevEnvironmentLayout.css';

// Panel Components
import MainAppPanel from './panels/MainAppPanel';
...
const DEFAULT_LAYOUT = {
  panels: [
    {
      id: 'main_app_panel',
      type: PANEL_TYPES.MAIN_APP_PANEL,
...
const WorkspaceLayout = () => {
  ...
  const calculateSnapZones = useCallback((draggingPanel, mousePosition) => {
    const zones = [];
    const ZONE_THRESHOLD = 50; // Distance to trigger snap zone
    ...
    layout.panels.forEach(panel => {
      ...
      // Calculate distance from mouse to panel edges
      const distances = {
  ...
  // Find the best snap zone based on mouse position
  ...
  const handleMouseDown = useCallback((e, panelId) => {
    ...
    // Check if clicking on header or drag-safe areas
    ...
    // Don't drag if clicking on controls or resize handles
  ...
  const handleMouseMove = useCallback((e) => {
    if (isDragging && dragData) {
      ...
      // Apply grid snapping if enabled and not snapping to zone
      if (layout.snapToGrid && !bestZone) {
        newPosition = snapPosition(newPosition);
      }

      // Immediate position update for fluid dragging
  ...
  // Function to accept layout suggestions
  ...
  // Function to dismiss layout suggestion
  ...
  // Enhanced drag preview with smooth transitions
  ...
  const renderPanelContent = (panel) => {
    switch (panel.type) {
      case PANEL_TYPES.MAIN_APP_PANEL:
        return <MainAppPanel />;
  ...
  // Auto-organize panels in optimal grid layout
  ...
  // Smart snap all panels to nearby positions
  const smartSnapAll = useCallback(() => {
    setLayout(prev => ({
      ...
      panels: prev.panels.map(panel => {
        ...
        // Find best snap targets for this panel
