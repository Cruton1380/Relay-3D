...
**[docs/architecture/RELAY-MASTER-BUILD-PLAN.md](docs/architecture/RELAY-MASTER-BUILD-PLAN.md)** is the single canonical document — the definitive system specification for Relay. Finalized 2026-02-19: 74 frozen contracts (original 27 + stage gate 28-53 + hardening 54-67 + identity/dispute/growth 68-74), 51 sections, 39-slice tiered build order across 8 tiers, constitutional hardening checklist (§50, 27/27 PASS), adversarial edge-case model (§49, 9 scenarios), sortition-based case resolution (§46), Password Dance somatic authentication (§48.2.1), guardian recovery (§48.2.2), invitation decay tree (§48.2.3), scheduled reverification by trust tier (§48.2.4), Cylindrical Bark Geometry model, 3-stage progressive unlock architecture, multi-resource economy, duels, spell taxonomy, detection mesh, founder key, fog of war, RPG attributes, unified color system, voice pipeline, engineering infrastructure (16 gaps), 4 worked use cases.
