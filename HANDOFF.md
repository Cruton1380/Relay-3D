...
**[docs/architecture/RELAY-MASTER-BUILD-PLAN.md](docs/architecture/RELAY-MASTER-BUILD-PLAN.md)** is the single canonical document — the definitive system specification for Relay. Finalized 2026-02-19: 85 frozen contracts (original 27 + stage gate 28-53 + hardening 54-67 + identity/dispute/growth 68-74 + sociological resilience 75-85), 51 sections, 39-slice tiered build order across 8 tiers, constitutional hardening checklist (§50, 37/37 PASS), adversarial edge-case model (§49, 15 scenarios including sociological), sortition with blind peer grading (§46), Password Dance with fallback (§48.2.1), guardian recovery (§48.2.2), invitation decay with community-governed refill rate (§48.2.3), scheduled reverification by trust tier (§48.2.4), Guardian Steward founder succession, emergency reform mechanism, physics-metaphor UX, regional feature voting, Cylindrical Bark Geometry model, 3-stage progressive unlock architecture, multi-resource economy, duels, spell taxonomy, detection mesh, founder key, fog of war, RPG attributes, unified color system, voice pipeline, engineering infrastructure (16 gaps), 4 worked use cases.
