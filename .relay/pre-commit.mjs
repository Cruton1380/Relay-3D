...
// Responsibilities:
//  - Compute staged/changed files between OLD_COMMIT and NEW_COMMIT
//  - If this commit changes .relay/pre-commit.mjs, require a signed commit
//  - Run validation.mjs in a restricted sandbox to enforce whitelist + validation
//  - On success, maintain a lightweight JSON index (relay_index.json) for changed meta.yaml
...
function requireSignedIfChangingPreCommit(changes) {
  const changedPreCommit = changes.some((e) => e.path === '.relay/pre-commit.mjs');
  if (!changedPreCommit) return;
  // If the commit modifies the pre-commit script, require a signed commit
  try {
    git(['verify-commit', NEW_COMMIT]);
    // Optional: verify presence of allowed keys directory (advisory)
    const sshDir = path.join(process.cwd(), '.relay', '.ssh');
    if (!fs.existsSync(sshDir)) {
      // still allow if verify-commit passed
      return;
    }
    // We could implement mapping of allowed signers; for now, verification success is sufficient.
  } catch (e) {
    console.error('Commit modifies .relay/pre-commit.mjs but is not signed (git verify-commit failed).');
    process.exit(1);
  }
}
...
function runValidationSandbox(changes) {
  ...
  try {
    ...
    // The script may export a global function validate(api) or set module.exports
    ...
    if (typeof validateFn !== 'function') {
      // If no explicit export, try calling default exported function
      validateFn = context.default || result?.default;
    }
    if (typeof validateFn !== 'function') {
      // As a last resort, assume the script invoked validation inline and set global.validationResult
    ...
    if (vr && typeof vr.then === 'function') {
      // Avoid async; not supported in hook
...
function upsertIndex(changes) {
  ...
  if (fs.existsSync(indexPath)) {
    try { index = JSON.parse(fs.readFileSync(indexPath, 'utf8')); } catch { index = { items: [] }; }
  }
  ...
  for (const ch of changes) {
    if (ch.path.endsWith('/meta.yaml') || ch.path.endsWith('/meta.yml') || ch.path.endsWith('meta.yaml') || ch.path.endsWith('meta.yml')) {
      ...
      if (!json) continue;
      const doc = { ...json, _branch: BRANCH, _meta_dir: metaDir, _updated_at: new Date().toISOString() };
...
function collectAllowedSigners() {
  ...
  const blobs = [];
  for (const d of dirs) {
    // list directory entries via git ls-tree
    let listing = '';
    try { listing = git(['ls-tree', '--name-only', `${NEW_COMMIT}:${d}`]); } catch { listing = ''; }
    if (!listing) continue;
  ...
  try { fs.writeFileSync(tmp, content); return tmp; } catch { return null; }
}
...
function verifyCommitWithAllowedSigners() {
  ...
  try {
    ...
    execFileSync('git', ['-c', `gpg.ssh.allowedSignersFile=${allowed}`, '-c', 'gpg.format=ssh', '--git-dir', GIT_DIR, 'verify-commit', NEW_COMMIT], { stdio: 'ignore' });
    return true;
  } catch {
    return false;
  } finally {
    try { if (allowed) fs.unlinkSync(allowed); } catch {}
  }
}

function main() {
  const changes = listChanged();
  requireSignedIfChangingPreCommit(changes);
