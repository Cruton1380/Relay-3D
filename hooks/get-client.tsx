 ...
 * JSX is transpiled by RepoBrowser using @babel/standalone with __ctx__.React.createElement pragma
 */
...
export default async function getClient(ctx: HookContext) {
  try {
    ...
    async function lazyLoadComponents(): Promise<void> {
      if (!tmdbClient) tmdbClient = await helpers.loadModule('./lib/sources/tmdb.ts');
    ...
    if (searchMatch) {
      ...
      try {
        const queryMod = await helpers.loadModule('./query-client.tsx');
    ...
    // Default: render file
    console.log('[get-client] Default route, path:', path)
