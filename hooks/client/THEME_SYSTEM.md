...
```js
import { registerThemeStyles } from '<repo>/apps/client-react-native/src/themedRuntime'
...
- Keep all platform-aware logic next to the client entrypoint. The template does not need to know whether it runs in React Native, a browser, or a future surface such as WebAssembly; it only emits themed-styler classes and relies on the context you provide.
